...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info('Word export requested', {
      component: 'export-word-api',
      userId,
      messageId,
      sessionId
    });
    ...
    // Generate filename
    const fileName = generateExportFileName(session.name, messageId, 'docx');
    ...
    }, 'Word document exported to Google Drive successfully');

  } catch (error) {
    return handleApiError(error, 'export-word-api');
  }
}